...
CREATE TABLE "transactions" (
  ...
  CONSTRAINT "transactions_commission_amount_check" CHECK ("commission_amount" >= 0),
...
CREATE TABLE "collections" (
  ...
  CONSTRAINT "collections_voided_by_fkey" FOREIGN KEY ("voided_by") REFERENCES "officers"("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE "disbursements" (
  ...
  CONSTRAINT "disbursements_voided_by_fkey" FOREIGN KEY ("voided_by") REFERENCES "officers"("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
...
SELECT
  ...
  GREATEST(LEAST(t.commission_amount, COALESCE(c.total, 0)), 0) AS commission_retained,
  (COALESCE(c.total, 0) - COALESCE(d.total, 0)
     - GREATEST(LEAST(t.commission_amount, COALESCE(c.total, 0)), 0)) AS officer_held_balance,
  (t.total_amount - COALESCE(c.total, 0)) AS amount_due_from_buyer,
  ((t.total_amount - t.commission_amount) - COALESCE(d.total, 0)) AS amount_due_to_seller,
  CASE
    WHEN COALESCE(c.total, 0) >= t.total_amount
         AND COALESCE(d.total, 0) >= (t.total_amount - t.commission_amount)
      THEN 'settled'
...
DECLARE
  ...
  new_status TEXT;
BEGIN
  SELECT t.total_amount, t.commission_amount INTO v_total, v_commission
  ...
  IF v_collected >= v_total AND v_disbursed >= (v_total - v_commission) THEN
    new_status := 'settled';
